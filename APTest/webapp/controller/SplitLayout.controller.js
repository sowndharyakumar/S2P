sap.ui.define([
	...
	return BaseController.extend("ui.incture.APTest.controller.SplitLayout", {
		...
		onInit: function () {
			...
			sPDFPath = jQuery.sap.getModulePath("ui.incture.APTest.css.pdfs", "/Test.pdf");
			...
			oPropertyModel = new sap.ui.model.json.JSONModel(jQuery.sap.getModulePath("ui.incture.APTest.model", "/propertyModel.json"));
			...
			// var oInvoiceModel = new sap.ui.model.json.JSONModel(jQuery.sap.getModulePath("ui.incture.APTest.model", "/invItemModel.json"));
					...
					jQuery
						.ajax({
							...
							success: function (result) {
								...
								oInvoiceModel.getData().messageCount = "0";
		...
		fnVendorSelected: function (oEvent) {
			...
			oInvoiceModel.setProperty("/oInvoiceHeaderDetails/vendorId", ui.incture.APTest.util.Formatter.removeZero(oSelectedItem.Vendor));
		...
		handleGroupDialogConfirm: function () {
			var oTable = this.byId("container-APTest---invoiceTask--grnSummaryTableId-container-APTest---invoiceTask--grnTab-0"),
		...
		fnRowActionAccAss: function () {
			if (!this.oAccAssignment) {
				this.oAccAssignment = sap.ui.xmlfragment("ui.incture.APTest.view.fragments.accAssignment", this);
		...
		onOpenMessage: function () {
			if (!this.InvoiceMessage) {
				this.InvoiceMessage = sap.ui.xmlfragment("ui.incture.APTest.view.fragments.message", this);
				this.getView().addDependent(this.InvoiceMessage);
			}
			var oInvoiceModel = this.getModel("oInvoiceModel");
			this.InvoiceMessage.setModel(oInvoiceModel, "oInvoiceModel");
			this.InvoiceMessage.open();
		},
		onCloseMessage: function () {
			this.InvoiceMessage.close();
		},
		onOpenAttachment: function () {
			if (!this.InvoiceAttachment) {
				this.InvoiceAttachment = sap.ui.xmlfragment("ui.incture.APTest.view.fragments.Attachment", this);
				this.getView().addDependent(this.InvoiceAttachment);
			}
			var attachmentModel = this.getModel("attachmentModel");
			this.InvoiceAttachment.setModel(attachmentModel, "attachmentModel");
			this.InvoiceAttachment.open();
		},
		onCloseAttchments: function () {
			this.InvoiceAttachment.close();
		},
		onOpenComment: function () {
			if (!this.commentsFrag) {
				this.commentsFrag = sap.ui.xmlfragment("ui.incture.APTest.view.fragments.comments", this);
				this.getView().addDependent(this.commentsFrag);
			}
			var oInvoiceModel = this.getModel("oInvoiceModel");
			this.commentsFrag.setModel(oInvoiceModel, "oInvoiceModel");
			this.commentsFrag.open();
		},
		onCloseComment: function () {
			this.commentsFrag.close();
		},
		...
		fnOpenLineItemMessages: function (oEvent) {
			...
			if (!this._oLineItemMessagePopover) {
				this._oLineItemMessagePopover = sap.ui.xmlfragment("ui.incture.APTest.view.fragments.lineItemMessages", this);
			...
			if (oEvent.getSource().getSelected()) {
				for (var i = 0; i < modelPOData.length; i++) {
					if (modelPOData[i].deleted == "true" || modelPOData[i].checkBoxSelection == true || modelPOData[i].checkboxVisible == "true") {
						this.getView().byId("container-APTest---invoiceTask--idtogglePOLine-" + sId).setSelected(true);
					} else {
						this.getView().byId("container-APTest---invoiceTask--idtogglePOLine-" + sId).setSelected(false);
						break;
					}
				}
			} else {
						...
						success: function (result) {
							...
							if (result.poHistory.length > 0) {
								...
								for (var i = 0; i < oGRNModel.getData().grnUIDtoList[length1].grnItemTotalUIList.length; i++) {
									oGRNModel.getData().grnUIDtoList[length1].grnItemTotalUIList[i].histType = ui.incture.APTest.util.Formatter.histValue(
			...
			$.ajax({
				...
				success: function (result, xhr, data) {
					...
					for (var i = 0; i < result.invoiceItems.length; i++) {
						if (result.invoiceItems[i].excpetionMessage && result.invoiceItems[i].excpetionMessage.length > 0) {
							oInvoiceModel.getData().messageCount = oInvoiceModel.getData().messageCount + result.invoiceItems[i].excpetionMessage.length;
							for (var j = 0; j < result.invoiceItems[i].excpetionMessage.length; j++) {
								aMessage.push({
									"poNumber": result.invoiceItems[i].matchDocNum,
									"poItem": result.invoiceItems[i].matchDocItem,
									"text": result.invoiceItems[i].excpetionMessage[j].text
								});
							}
						}
					}
					oInvoiceModel.getData().messageCount = "" + oInvoiceModel.getData().messageCount;
					oInvoiceModel.getData().aMessage = aMessage;
					this.closeBusyDialog();
		...
		onReject: function () {
			...
			if (!this.rejectFragment) {
				this.rejectFragment = sap.ui.xmlfragment("ui.incture.APTest.view.fragments.rejectDialog", this);
		...
		getPdfData: function () {
			...
			$.ajax({
				...
				success: function (result, xhr, data) {
					...
					attachmentModel.getData().count = result.attachmentList.length > 0 ? "" + result.attachmentList.length : "0";
		...
		getComments: function () {
			...
			$.ajax({
				...
				success: function (result, xhr, data) {
					...
					if (result.commentDtos)
						oInvoiceModel.getData().commentsCount = oInvoiceModel.getData().comments.length > 0 ? "" + oInvoiceModel.getData().comments.length :
						"0";
					else
						oInvoiceModel.getData().commentsCount = "0";
					oInvoiceModel.refresh();
				}.bind(this)
			});
			// }
			...
			if (dFlag) {
				...
				if (invLineData) {
					for (var i = 0; i < invLineData.length; i++) {
						var id = "container-APTest---invoiceTask--toggleBtn-container-APTest---invoiceTask--idInvoiceTable-" + i;
		...
		onPressManualMatching: function () {
			// var oFragmentName = "ui.incture.APTest.view.fragments.manualMatching";
		...
		fnMatchBtnPress: function (oEvent) {
			...
			var oInvoiceModel = this.getView().getModel("oInvoiceModel"),
				invoiceData = oInvoiceModel.getData();
			var oInvItemSel = sap.ui.getCore().byId("container-APTest---invoiceTask--idUMInvTbl").getSelectedItem();
			if (oInvItemSel) {
				var objectIsNew = jQuery.extend({}, invoiceData.oInvoiceHeaderDetails);
				delete objectIsNew.invoiceItems;
				...
				$.ajax({
					...
					success: function (result, xhr, data) {
						...
						this._getUnmatchedInvItems();
						this._getUnmatchedPoItems();
						sap.ui.getCore().byId("container-APTest---invoiceTask--idUMInvTbl").removeSelections();
						sap.ui.getCore().byId("container-APTest---invoiceTask--idMatInvTbl").removeSelections();
						this.closeBusyDialog();
					}.bind(this),
					error: function (result, xhr, data) {
						this.closeBusyDialog();
		...
		fnConfirmUM: function (oEvent) {
			this.openBusyDialog();
			var oInvMatchedTbl = sap.ui.getCore().byId("container-APTest---invoiceTask--idMatInvTbl");
			var oInvoiceModel = this.getView().getModel("oInvoiceModel"),
				invoiceData = oInvoiceModel.getData();
		...
		fnFilterTableData: function (oEvent) {
			...
			if (oEvent) {
				...
				if (sSearchFieldId == "idMatchedInvoice") {
					sId = "container-APTest---invoiceTask--idMatInvTbl";
					...
					if (sap.ui.getCore().byId("container-APTest---invoiceTask--idUnmatchedInvoiceSelect").getSelectedKey() == "all") {
						aMetaModel = ["itemText", "vendorMaterialId", "upcCode"];
					} else {
						aMetaModel = [sap.ui.getCore().byId("container-APTest---invoiceTask--idUnmatchedInvoiceSelect").getSelectedKey()];
					}
					sId = "container-APTest---invoiceTask--idUMInvTbl";
				} else {
					if (sap.ui.getCore().byId("container-APTest---invoiceTask--idUnmatchedPOSelect").getSelectedKey() == "all") {
						aMetaModel = ["shortText", "poVendMat", "poUPC", "documentNumber", "documentItem"];
					} else {
						aMetaModel = [sap.ui.getCore().byId("container-APTest---invoiceTask--idUnmatchedPOSelect").getSelectedKey()];
					}
					sId = "container-APTest---invoiceTask--idPoTbl";
				}
		...
		fnBalanceCalculation: function () {
			...
			if (modelData.nonPoDtoList && modelData.nonPoDtoList.length > 0) {
				...
				modelData.oInvoiceHeaderDetails.grossAmount = parseFloat(modelData.oInvoiceHeaderDetails.invoiceLineTotal) - this.getNumberValue(
					modelData.oInvoiceHeaderDetails.disAmt) + this.getNumberValue(modelData.oInvoiceHeaderDetails.deposit) + this.getNumberValue(
					modelData.oInvoiceHeaderDetails.taxAmount);
				modelData.oInvoiceHeaderDetails.grossAmount = parseFloat(modelData.oInvoiceHeaderDetails.grossAmount).toFixed(3);
			}
		...
		onSearcAttach: function (oEvent) { // add filter for search
			...
			var reqList = sap.ui.getCore().byId("attachListItems");
		...
		onBeforeUploadStarts: function (oEvent) {
			...
			reader.onload = function (event) {
				...
				attachmentModel.getData().count = attachmentModel.getData().docManagerDto.length > 0 ? "" + attachmentModel.getData().docManagerDto
					.length :
					"0";
				attachmentModel.refresh();
			};
